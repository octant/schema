...
describe('NumberValidator', () => {
  ...
  const customMessageValidator = new NumberValidator(fieldDefinitionWithCustomMessage)
  ...
  test('returns an array of error messages for invalid values', () => {
    expect(validator.validate('1', fields)).toEqual(['should not be less than 2'])
    expect(validator.validate('8', fields)).toEqual(['should not be more than 6'])
  })

  test('returns a custom message when prompt is present and validation fails', () => {
    expect(customMessageValidator.validate('9', fields)).toEqual(['should be between 2 and 6'])
  })
})
