...
describe("Validator", () => {
  ...
  test("returns an array of error messages", () => {
    ...
    expect(validator.validate("value", fields)).toEqual([
      "improperly formatted"
    ]);
    expect(validator.validate("test value", invalidField)).toEqual([
  ...
  test("returns a custom message if prompt is included in definition", () => {
    ...
    expect(result).toContain(
      'must contain "test" and age has to be less than 40'
    );
